...
//Nicht mehr benötigte Setup Funktion
...
//Funktion zur Erstellung der optischen Darstellung des Displays
extern "C" void process_and_show()
{
    initArduino();

    //Initialisierung für Das E-Ink Display mit den Massen 7,5 Inch
    DEV_Module_Init();
    EPD_7IN5_V2_Init();
    EPD_7IN5_V2_Clear();

    //Speicherreservierung des Bildes
    ...
    //Kürzung von Strings, die zu lang für den vorgesehenen Platz auf dem Display sind:
    //Kürzung vom Titel des aktuellen Meetings     
    char aktMeetTit[100];
    strcpy(aktMeetTit, doc["aktuellesMeeting"]["titel"]);
    aktMeetTit[29]=0;
    //Kürzung vom Verantwortlichen des aktuellen Meetings
    char aktMeetVer[100];
    strcpy(aktMeetVer, doc["aktuellesMeeting"]["verantwortlicher"]);
    aktMeetVer[28]=0;
    //Kürzung vom Topic
    char mytopic[100];
    strcpy(mytopic, topic_in_buf);
    mytopic[7]=0;    
            

    // Bild beschriften:
    // optisch ansprechende Einteilung in Regionen
    ...
    //Füllen der oberen Hälfte des Displays
    ...
    TUERSCHILD_LOGI(tag, "wrote akutelles meeting time");

    //Füllen der unteren Hälfte des Displays
    ...
    TUERSCHILD_LOGI(tag, "wrote following meeting1");
    ...
    TUERSCHILD_LOGI(tag, "wrote following meeting2");
    ...
    TUERSCHILD_LOGI(tag, "wrote following meeting4");
    ...
    Paint_DrawString_EN(630, 100, mytopic, &Font24, BLACK, WHITE);
